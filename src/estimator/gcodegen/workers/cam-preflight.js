...
let { settings, opIndex, op, geometry, openGeometry, tabGeometry, documents } = workerData;
...
while (jobs.length) {
    try {
        ...
        if (job) job(() => {
            ...
            parentPort.postMessage({ event: "onProgress", percent: parseInt(percent) })
        });
    } catch (error) {
        console.error(error)
        parentPort.postMessage({ event: "onError", message: "Something wrong has happened, sorry.", level: "error", error: error.toString() })
    }
}

parentPort.postMessage({ event: "onDone", settings, opIndex, op, geometry, openGeometry, tabGeometry, filteredDocIds, docsWithImages })